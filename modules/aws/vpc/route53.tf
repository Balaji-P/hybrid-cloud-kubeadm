...
resource "aws_route53_record" "vpc_one_etcd" {
  zone_id = "${aws_route53_zone.dlos_vpc.zone_id}"
  name    = "etcd.dlos-example.com"
  type    = "A"
  ttl     = "300"
  records = ["${substr((cidrsubnet(aws_subnet.vpc_one_controller.cidr_block,8,11)),0, 9)}","${substr((cidrsubnet(aws_subnet.vpc_two_controller.cidr_block,8,11)),0, 9)}"]
}
